...
Detection of computed attribute names is deliberately shallow: it looks for `getattr`, `setattr`, `hasattr`, and `delattr` called with a name Privata cannot read as a literal.
